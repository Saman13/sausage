...
if [ "$green_running" = "true" ] && [ "$green_status" = "healthy" ]; then
    ...
    docker --context remote stop $GREEN_CONTAINER
    docker --context remote rm $GREEN_CONTAINER
    ...
    docker --context remote stop $BLUE_CONTAINER
    docker --context remote rm $BLUE_CONTAINER
